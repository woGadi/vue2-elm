      ...
      <van-swipe class="my-swipe" :loop="false">
        ...
        <van-swipe-item>
          ...
          <van-grid :column-num="4" clickable icon-size="1.2rem">
            <!-- 循环渲染后8项 -->
            <van-grid-item v-for="item in cateList.slice(cateList.length / 2)" :key="item.id" :icon="item.image_url" :text="item.title" to="/category" @click="sendCateTitle(item.title)"></van-grid-item>
          </van-grid>
        </van-swipe-item>
        <!-- 轮播的第二页 -->
        <van-swipe-item>
          ...
          <van-grid :column-num="4" clickable icon-size="1.2rem">
            <!-- 循环渲染前8项 -->
            <van-grid-item v-for="item in cateList.slice(0, cateList.length / 2)" :key="item.id" :icon="item.image_url" :text="item.title" to="/category" @click="sendCateTitle(item.title)"></van-grid-item>
          </van-grid>
        </van-swipe-item>
      </van-swipe>
  ...
  methods: {
    ...
    async getStoreList() {
      ...
      if (this.loadListParams.isLoading) {
        // 下拉刷新：新数据在前
        this.storeList = [...res.data, ...this.storeList]
      ...
      console.log(this.storeList, 2017)
    },
    // 监听加载更多店家列表的事件
    loadStoreList() {
